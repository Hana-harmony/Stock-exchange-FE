...
현재 프론트엔드 초안은 백엔드 작업자가 기능 확인용으로 구현해 둔 상태입니다.

이번 작업의 목표는 기존 기능, API 연동, 라우팅, 상태관리, 데이터 흐름은 최대한 유지하면서 Figma 디자인 시스템 기준으로 UI/UX를 전면 교체하는 것입니다.

새로운 기능을 임의로 추가하거나 API 구조를 추측하지 말고, 기존 코드의 동작을 보존한 상태에서 화면 디자인, 레이아웃, 컴포넌트 구조, 스타일만 Figma 기준으로 리팩토링해 주세요.
...
UI 교체 중 불가피하게 변경이 필요한 경우, 변경 이유와 영향 범위를 반드시 보고해 주세요.
...
## 6. 기존 화면과 Figma 화면 매핑

기존 프론트 코드에 있는 화면을 Figma 화면과 매핑해서 작업해 주세요.

작업 전 또는 작업 완료 후 아래 형식으로 정리해 주세요.

| 기존 화면/파일 | 대응 Figma 화면 | 작업 상태 | 비고 |
| --- | --- | --- | --- |
| `HomeScreen` | 홈 화면 | 완료 | Header, Bottom Navigation 교체 |
| `LoginScreen` | 로그인 화면 | 완료 | Input, Button 컴포넌트 적용 |
| `ReceiptDetailScreen` | 영수증 상세 화면 | 확인 필요 | Figma 화면 없음 |
...
작업 완료 후, 각 화면별 Frame 링크를 화면명과 함께 정리해서 안내해 주세요.
...
2. 회원가입 화면: `[Figma 링크]`
3. 홈 화면: `[Figma 링크]`
4. 영수증 촬영 화면: `[Figma 링크]`
5. OCR 결과 수정 화면: `[Figma 링크]`
6. 소비 내역 상세 화면: `[Figma 링크]`
7. 월간 리포트 화면: `[Figma 링크]`
8. 더치페이 화면: `[Figma 링크]`
9. 마이페이지 화면: `[Figma 링크]`
10. 공통 컴포넌트 페이지: `[Figma 링크]`
