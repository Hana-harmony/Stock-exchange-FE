...
세무 흐름은 실제 파일 선택, 세 문서 순차 업로드, OCR 진행·실패·검수 상태와 개인정보 마스킹을 함께 검증한다. 진행률 100%는 완료 판정 근거가 아니며, `VERIFIED`·`REVIEW_REQUIRED`·`REJECTED`·`FAILED` 상태만 폴링을 종료한다. 문서 제출 후 백오피스 상태 동기화가 실패해도 OCR 화면에 머물지 않고 제출 완료·재시도 상태를 표시한다. 제출 완료 화면의 `Confirm`·`Submit Again`은 전체 폭 액션으로 렌더링하고, 환급 대상 실현이익이 없으면 `USD 0.00`대신 무환급 사유를 표시한다.
