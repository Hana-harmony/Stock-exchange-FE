...
# 2026-07-21 · 전문 번역 완료 후 상세 제공

- 완전한 영문 전문이 준비된 이벤트만 목록에 노출되도록 API 계약을 변경했다.
- 상세 화면의 5초 반복 재조회·15분 대기·번역 준비 문구를 제거했다.
- 목록 데이터의 전문을 즉시 렌더링하고 상세 API는 한 번만 새로고침한다.
- 이미 번역된 전문은 상세 갱신 요청 중에도 즉시 표시하고 빈 응답으로 기존 전문을 덮어쓰지 않는다.
